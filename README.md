...
In this project we use a PID controller to controll the steering angle of the car based on the cross track error (cte). A PID controller consists of three different parameter which have different influence on the result. The goal is to controll the steering angle of the car in a way so that the car stays as close to the center of the road as possible. Therefore the cte is simply the offset of the car to the lane centerline.

**P** - The proportional part of the PID controller. This part is used to immediately counteract the instantanious error. This component acts, as the name implys, proportional to the error. In our case this means, the larger the cte the steeper the resulting steering angle.

**I** - The integral part of the PID controller. In contrary to the P component the I part is not calculated based on the cross track error but the sum (integral) of the cte over the last n steps. This helps to counteract systematic errors introduces for example by a constant drift.
